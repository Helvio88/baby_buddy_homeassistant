...
from .const import MOCK_BABY_SENSOR_ID, MOCK_ACTION_ADD_CHILD_SCHEMA


@pytest.mark.usefixtures("setup_baby_buddy_entry_live")
async def test_action_add_child(
    hass: HomeAssistant,
) -> None:
    """Test the "add child" action call."""

    entity_id = MOCK_BABY_SENSOR_ID
    await hass.services.async_call(
        DOMAIN,
        ATTR_ACTION_ADD_CHILD,
        MOCK_ACTION_ADD_CHILD_SCHEMA,
        blocking=True,
    )
    ...
    assert (
        ...
        == MOCK_ACTION_ADD_CHILD_SCHEMA[ATTR_FIRST_NAME]
    )
    assert state.attributes[ATTR_ICON] == ATTR_ICON_CHILD_SENSOR
    assert (
        state.attributes[ATTR_LAST_NAME]
        == MOCK_ACTION_ADD_CHILD_SCHEMA[ATTR_LAST_NAME]
    )
